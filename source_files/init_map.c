...
/*   Updated: 2024/01/09 18:14:59 by abolor-e         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_add_gnlmap(char **line_tmp, char *next_line)
{
	...
	if (!*line_tmp || !next_line)
		return (NULL);
	string = ft_calloc((ft_strlen(*line_tmp) + 
				ft_strlen(next_line) + 1), sizeof(char));
	if (!string)
		return (NULL);
	ft_strlcpy(string, *line_tmp, ft_strlen(*line_tmp) + 1);
	ft_strlcat(string, next_line, 
		ft_strlen(*line_tmp) + ft_strlen(next_line) + 1);
	free(*line_tmp);
	return (string);
}
